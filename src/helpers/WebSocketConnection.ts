...
class WebSocketConnection {
    ...
    */
    check = () => {
        if (!this._wsConnection || this._wsConnection.readyState === WebSocket.CLOSED) this.connect(); //check if websocket instance is closed, if so call `connect` function.
    };
    ...
    */
    connect = () => {
        ...
        this._wsConnection.onclose = e => {
            console.log(
                `Connection to ${this._serverAddress} server closed. Reconnect will be attempted in ${Math.min(10000 / 1000, (that._timeout + that._timeout) / 1000)} seconds`,
                e.reason
            );
    ...
    constructor(serverAddress: string, onOpenCallback: () => void, onMessageCallback: (evt: MessageEvent) => void) {
